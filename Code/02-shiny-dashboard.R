...
server <- function(input, output, session) {
  ...
  output$stackedBarGraph <- renderPlotly({
    data_to_plot <- filtered_workforce_data()  # Call the reactive expression here
  ...
  output$downloadGraphsWordworkforce <- downloadHandler(
    ...
    content = function(file) {
      ...
      first_graph_ggplot <- ggplot(filtered_workforce_data(), aes(x = country_name, y = value_percentage, fill = indicator_name)) +
      ...
      second_graph_ggplot <- ggplot(filtered_data %>%
                                      filter(year %in% c(first_year, last_year)) %>%
                                      group_by(year, indicator_name) %>%
                                      summarise(value_percentage = mean(value_percentage, na.rm = TRUE), .groups = "drop"),
                                    aes(x = value_percentage, y = factor(year, levels = c(last_year, first_year)), fill = indicator_name)) +
